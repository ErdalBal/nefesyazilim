...
<div class="card">
    <div class="card-body">
        <div class="d-lg-flex align-items-center mb-4 gap-3">
          ...
          <div class="ms-auto"><a href="{{route('portfolio.create')}}" class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Portfolio Ekle</a></div>
...
<div class="table-responsive">
    ...
    <table class="table mb-0">
        ...
        <tbody>
            @foreach ($portfoliolist as $key => $item)
            <tr>
                <td>
                ...
                <td>
                    ...
                    <img width="100" height="75" src="{{asset('backend/uploads/portfolio/'.$item->file)}}" alt="">
                </td>
                <td>{{$item->title}}</td>
                <td>{{$item->subtitle}}</td>
                <td>{{$item->client}}</td>
                <td>{{$item->client_web}}</td>   
                <td>
                ...
                <td>
                    <div class="d-flex order-actions">
                        <a href="{{route('portfolio.edit',$item->id)}}" class=""><i class='bx bxs-edit'></i></a>
    ...
    <nav style="margin-top: 25px;position: absolute;right: 0px;" aria-label="paginate">
        <ul class="pagination pagination-sm">
            @if ($portfoliolist->onFirstPage())
            <li class="page-item disabled"><a class="page-link" href="javascript:;" tabindex="-1" aria-disabled="true">Önceki</a>
            </li>
        @else
        <li class="page-item"><a class="page-link" href="{{ $portfoliolist->previousPageUrl() }}" tabindex="-1" >Önceki</a>
        </li>
    ...
    {{-- Sayfa Numaraları --}}
    @foreach ($portfoliolist->links()->elements[0] as $page => $url)
    @if ($page == $portfoliolist->currentPage())
...
@endforeach
 ...
 {{-- Sonraki Sayfa --}}
 @if ($portfoliolist->hasMorePages())
 <li class="page-item"><a class="page-link" href="{{ $portfoliolist->nextPageUrl() }}">Sonraki</a></li>
@else
...
@section('js')
    <script>
          ...
          var deleteUrl = "{{ route('portfolio.delete', ':id') }}";
        $(function(){
